...
def enquiries_get():
    ...
    if search != '':
        ...
        enquiries = Enquiries.query.filter_by(user=current_user.id).filter(Enquiries.course.in_(course_ids)).order_by(Enquiries.id.desc()).paginate(page=page, per_page=rowsPerPage)
    else:
